...
> **Archived build input:** This prompt records the repository's original implementation brief and is not the current architecture contract. Do not implement its direct-actuation instructions. The authoritative runtime is `deepfield-fleet -> GCL -> fleet-llm-d`; GCL submits signed advisory DecisionPackage CloudEvents to fleet `/api/v2/intents`, fleet owns authorization and actuation, and `are-immutable-ledger` provides evidence/proof only. See `docs/READFIRST.md`.
